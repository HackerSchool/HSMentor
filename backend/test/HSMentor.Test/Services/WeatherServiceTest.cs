...
public class WeatherServiceTest
{
    ...
    */
    [Fact]
    public void CreateForecastAndCheckIfExists()
    {
        // Get service instance
        var weatherService = BuildWeatherService();

        // Create a weather forecast
        var weatherForecastCreated = weatherService.CreateForecast();

        // Try to get forecast with the id from previous creation
        var weatherForecastGotten = weatherService.GetForecast(weatherForecastCreated.Id);

        // Check if they are the same object
        Assert.Equal(weatherForecastGotten, weatherForecastCreated);

        // Get list of all forecasts
        var weatherForecastList = weatherService.GetAllForecasts();
